...
let isSwitchingCamera = false; // Add this flag to manage camera switching state
...
const cameraSelect = document.getElementById('camera-select'); // Added for camera selection
...
// 初始化攝影機
async function initCamera(deviceId = null) { // Added deviceId parameter
  try {
    ...
    if (error.name === "NotAllowedError" || error.name === "PermissionDeniedError") {
      message = "攝影機存取被拒絕。請檢查瀏覽器權限設定。";
    } else if (error.name === "NotFoundError" || error.name === "DevicesNotFoundError") {
      message = "找不到可用的攝影機裝置。";
    } else if (error.name === "NotReadableError" || error.name === "TrackStartError") {
      message = "攝影機目前無法讀取，可能正被其他應用程式使用中。";
    } else if (error.name === "OverconstrainedError" || error.name === "ConstraintNotSatisfiedError") {
      message = deviceId ? "所選鏡頭不支援請求的解析度或設定。" : "攝影機不支援請求的解析度或設定。";
    } else if (error.name === "AbortError") {
      message = "攝影機啟動被中止。";
    } else {
      message = `攝影機啟動失敗: ${error.message}`;
    }
...
async function populateCameraSelect() {
  if (!navigator.mediaDevices || !navigator.mediaDevices.enumerateDevices) {
    showToast("瀏覽器不支援枚舉媒體設備功能。", "warning");
    cameraSelect.style.display = 'none'; // Hide select if not supported
    return;
  }
  try {
    ...
    // Add event listener for when a new camera is selected
    cameraSelect.addEventListener('change', async (event) => {
      const selectedDeviceId = event.target.value;
      console.log(`Camera selected: ${selectedDeviceId}`);
      isSwitchingCamera = true; // Set flag to true before starting camera switch
      showToast('正在切換鏡頭...', 'info');

      try {
        await initCamera(selectedDeviceId); // initCamera stops old stream, starts new, waits for metadata
        resizeCanvas(); // Resize canvas for the new camera stream
        showToast('鏡頭切換成功!', 'success');
        console.log('Camera switch successful, processing will resume.');
      } catch (error) {
        console.error('切換鏡頭失敗:', error);
        showToast(`切換鏡頭失敗: ${error.message}`, 'error', 5000);
        // Optionally, attempt to revert to a previous camera or handle the error state
      } finally {
        // Defer setting the flag to false to the next animation frame
        // This allows the browser a cycle to stabilize video state if needed.
        requestAnimationFrame(() => {
          isSwitchingCamera = false;
          console.log('isSwitchingCamera set to false. Draw loop will resume sending frames normally.');
        });
      }
    });

  } catch (error) {
    console.error("枚舉攝影機裝置時發生錯誤:", error);
    showToast("無法獲取攝影機列表。", "error");
    cameraSelect.style.display = 'none';
  }
}
...
async function init() {
  try {
    setupEventListeners();
    localStorage.clear(); 
    await populateCameraSelect(); // Populate camera dropdown first
    // Initialize with the first camera in the list, or default if no specific one is selected yet
    const initialDeviceId = cameraSelect.options.length > 0 ? cameraSelect.value : null;
    await initCamera(initialDeviceId);
    resizeCanvas();
...
async function drawLoop() {
  if (isSwitchingCamera) {
    // If camera is currently being switched, skip processing and wait for the next frame.
  ...
  if (!video.srcObject || video.readyState < video.HAVE_CURRENT_DATA || 
    ...
    // If video is not ready (no source, not enough data, dimensions are zero, paused, or ended),
    // skip sending the frame and wait for the next animation frame.
    requestAnimationFrame(drawLoop);
    return;
  }
  ...
  try {
    ...
    console.error('處理影像錯誤 (drawLoop):', error); // More specific console error
    // Show a toast to the user about the processing error.
    // If this error is persistent, further investigation or a more robust recovery (like re-initializing MediaPipe) might be needed.
    showToast(`影像處理時發生錯誤: ${error.message}`, 'error', 3000);
  }
  requestAnimationFrame(drawLoop);
}
